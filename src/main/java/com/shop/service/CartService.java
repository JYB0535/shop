...
public class CartService {
    ...
    //장바구니 추가 기능 구현 메서드                 //유저 정보
    public Long addCart(CartItemDto cartItemDto, String email) {
        /* 순서 정확하게 하기
        1.Item 엔티티 조회
        2.Member 엔티티 조회 (누구의 카트냐)
        3.Cart 엔티티 조회 ==> 카트가 없다(한번도 장바구니 기능을 쓴 적이 없다) ==> Cart 엔티티 저장(만들어줌)
        4.CartItem 엔티티 조회
        4-1 없다 ==> CartItem  엔티티 저장
        4-2 있다 ==> 이미 내 카트에 그 품목이 존재한다? ==> 조회된 CarItem 엔티티 count 증가
        */
    ...
    public List<CartDetailDto> getCartList(String email) {
        ...
        */
        ...
        Cart cart = cartRepository.findByMember(member);
        if(cart == null) {
            return new ArrayList<>(); //아무 것도 안 들어있는 사이즈 0 리스트
        }
        //특정 Cart에 들어있는 CartItem 리스트 조회
    ...
    public Long orderCartItems(List<CartOrderDto> cartOrderDtoList, String email) {
        // 책임1. CartItemDto에 있는 cartItemId를 이용 ==> OrderDtoList를 진행
        ...
        // 주문 ==> 수행 ==> orderService ==> OrderDtoList, email ///orderService에 위임
